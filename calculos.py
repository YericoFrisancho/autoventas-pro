...
def generar_cronograma_credito(precio_vehiculo, cuota_inicial_porc, tea, plazo_meses,
    ...
    # Cálculo de Seguros Fijos (Solo se cobrarán en periodo regular)
    ...
    # Vector de flujos para calcular VAN y TIR (Momento 0 = ingreso del capital negativo para el banco, o positivo para el cliente)
    ...
    for t in range(1, plazo_meses + 1): 
        ...
        # NUEVA LÓGICA DE GRACIA (A petición del profesor)
        if t <= meses_gracia and tipo_gracia != 'NINGUNO':
            if tipo_gracia == 'TOTAL':
                cuota_base_t = 0.0 
                amortizacion_t = 0.0 
                seguros_t = 0.0       # No se pagan seguros
                pago_total_t = 0.0    # No se paga absolutamente nada
                saldo_vigente += interes_t 
            elif tipo_gracia == 'PARCIAL':
                cuota_base_t = interes_t 
                amortizacion_t = 0.0 
                seguros_t = 0.0       # No se pagan seguros
                pago_total_t = cuota_base_t  # Se paga SOLO el interés
        else:
            # Cálculo normal (Fórmula francesa estándar ajustada)
            plazo_r = plazo_meses - t + 1 
            seguros_t = total_seguros_mensuales # Aquí sí se cobran los seguros
            ...
            pago_total_t = cuota_base_t + seguros_t
